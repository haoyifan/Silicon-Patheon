...
def _configure_server_logging(level: str, log_file: Path | None) -> Path:
    ...
    """
    if log_file is None:
        log_dir = Path.home() / ".clash-of-odin" / "logs"
...
def main() -> int:
    p = argparse.ArgumentParser(description="Run the clash-of-odin backend")
    ...
    p.add_argument(
        ...
        help=(
            ...
            "~/.clash-of-odin/logs/server-<pid>-<timestamp>.log"
        ),
    )
    args = p.parse_args()
    ...
    from clash_of_odin.server.heartbeat import run_sweep_loop
